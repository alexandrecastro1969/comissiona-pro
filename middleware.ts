...
export function middleware(request: NextRequest) {
  ...
  // const nonce = Buffer.from(crypto.randomUUID()).toString('base64'); // Comentado

  // CSP mais permissiva para desenvolvimento
  // const isDevelopment = process.env.NODE_ENV === 'development'; // Comentado
  
  // const cspHeader = ` // Comentado
  //   default-src 'self';
  ...
  if (pathname === '/') {
    ...
    // response.headers.set( // Comentado
    //   'Content-Security-Policy',
    //   cspHeader.replace(/\s{2,}/g, ' ').trim()
    // );
    // Passar o nonce para a aplicação
    // response.headers.set('x-nonce', nonce); // Comentado
    return response;
  }
